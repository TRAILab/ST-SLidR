...
class DilationFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, preprocessing=None):
        ...
        assert (
            config["model"]["image"]["encoder"] == "resnet50"
        ...
        if config["model"]["image"]["weights"] == "imagenet":
            self.encoder.load_state_dict(model_zoo.load_url(model_urls["resnet50"]))

        weights = adapt_weights(architecture=config["model"]["image"]["weights"])
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(in1, config["model"]["n_out"], 1),
            nn.Upsample(scale_factor=4, mode="bilinear", align_corners=True),
        )
        self.preprocessing = preprocessing
        self.normalize_feature = config["model"]["normalize_features"]
        if "return_pretrain_feat" in config["model"]["image"]:
            self.return_pretrain_feat = config["model"]["image"]["return_pretrain_feat"]
        else:
            self.return_pretrain_feat = False

    def forward(self, x):
        if self.preprocessing:
            x = self.preprocessing(x)
        pretrained_feat = self.encoder(x)
        x = self.decoder(pretrained_feat)
        if self.normalize_feature:
            x = F.normalize(x, p=2, dim=1)

        if self.return_pretrain_feat:
            return {
            "features": pretrained_feat,
            "embeddings": x
            }
        else:
            return {"embeddings": x}
...
class PPKTFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, preprocessing=None):
        ...
        self.image_config = config["model"]["image"]
        Encoder = resnet_encoders[self.image_config["encoder"]]["encoder"]
        params = resnet_encoders[self.image_config["encoder"]]["params"]
        self.encoder = Encoder(**params)

        if self.image_config["weights"] == "imagenet":
            self.encoder.load_state_dict(model_zoo.load_url(model_urls[self.image_config["encoder"]]))

        if self.image_config["weights"] not in (None, "imagenet"):
            assert (
                self.image_config["encoder"] == "resnet50"
            ), "{} weights are only available for resnet50".format(
                self.image_config["weights"]
            )
            weights = adapt_weights(architecture=self.image_config["weights"])
            if weights is not None:
                self.encoder.load_state_dict(weights)
        ...
        if self.image_config["encoder"] == "resnet18":
            in1 = 512
        elif self.image_config["encoder"] == "resnet50":
            in1 = 2048
...
class DinoVitFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, preprocessing=None):
        ...
        assert (
            self.image_config["encoder"] in dino_models.keys()
        ), f"DilationFeatureExtractor is only available for {dino_models.keys()}"

        model_name, patch_size, embed_dim = dino_models[self.image_config["encoder"]]
        ...
        path_to_pretrained_weights = "weights/"+ self.image_config["weights"] + ".pth"
        assert os.path.isfile(path_to_pretrained_weights)
        dino_vit.load_pretrained_weights(self.encoder, path_to_pretrained_weights, None, model_name, patch_size)

        for param in self.encoder.parameters():
            param.requires_grad = False

        self.decoder = nn.Sequential(
            nn.Conv2d(embed_dim, config["model"]["n_out"], 1),
    ...
    def forward(self, x):
        ...
        pretrained_feat = x[:, 1:, :].contiguous().transpose(1, 2).contiguous().view(batch_size, self.embed_dim, f_height, f_width)
