...
def main():
    ...
    model = make_model(config, config["trainer"]["pretraining_path"]).to(0)
    checkpoint = torch.load(config["trainer"]["resume_path"], map_location=torch.device(0))
    if "config" in checkpoint:
        for cfg in ("voxel_size", "cylindrical_coordinates"):
            assert checkpoint["config"]["dataset"][cfg] == config["dataset"][cfg], (
                f"{cfg} is not consistant.\n"
                f"Checkpoint: {checkpoint['config']['dataset'][cfg]}\n"
                f"Config: {config[cfg]}."
            )
    try:
        model.load_state_dict(checkpoint["model"]["pointcloud"]["backbone"])
    except KeyError:
        weights = {
            k.replace("model.", ""): v
