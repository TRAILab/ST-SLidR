...
def make_transforms_clouds(config):
    ...
    """
    transforms = []
    if config["dataset"]["transforms"]["clouds"] is not None:
        for t in config["dataset"]["transforms"]["clouds"]:
            if t.lower() == "rotation":
...
class ComposeAsymmetrical:
    ...
    def __call__(self, pc, features, img, pairing_points, pairing_images, superpixels=None, bg_masks=None):
        for transform in self.transforms:
            pc, features, img, pairing_points, pairing_images, superpixels, bg_masks = transform(
                pc, features, img, pairing_points, pairing_images, superpixels, bg_masks
            )
        if superpixels is None:
            if bg_masks is None:
                return pc, features, img, pairing_points, pairing_images, None, None
            else:
                return pc, features, img, pairing_points, pairing_images, None, bg_masks
        elif bg_masks is None:
            return pc, features, img, pairing_points, pairing_images, superpixels, None
        else:
            return pc, features, img, pairing_points, pairing_images, superpixels, bg_masks
...
class ResizedCrop:
    ...
    def __call__(self, pc, features, images, pairing_points, pairing_images, superpixels, bg_masks=None):
        imgs = torch.empty(
        ...
        if bg_masks is not None:
            bg_masks = bg_masks.unsqueeze(1)
            sps_bg_mask = torch.empty(
                (images.shape[0],) + tuple(self.crop_size), dtype=torch.uint8
            )
        ...
        if self.crop_center:
            ...
            for id, img in enumerate(images):
                ...
                if bg_masks is not None:
                    sps_bg_mask[id] = resize(
                        bg_masks[id], self.crop_size, InterpolationMode.NEAREST
                    )
        ...
        if superpixels is None:
            if bg_masks is None:
                return pc, features, imgs, pairing_points_out, pairing_images_out, superpixels, bg_masks
            else:
                return pc, features, imgs, pairing_points_out, pairing_images_out, superpixels, sps_bg_mask
        elif bg_masks is None:
            return pc, features, imgs, pairing_points_out, pairing_images_out, sps, bg_masks
        else:
            return pc, features, imgs, pairing_points_out, pairing_images_out, sps, sps_bg_mask
    ...
    def __call__(self, pc, features, images, pairing_points, pairing_images, superpixels, bg_masks=None):
        ...
        for i, img in enumerate(images):
            if random.random() < self.p:
                ...
                if bg_masks is not None:
                    bg_masks[i] = hflip(bg_masks[i: i + 1])
                mask = pairing_images[:, 0] == i
                pairing_images[mask, 2] = w - 1 - pairing_images[mask, 2]

        return pc, features, images, pairing_points, pairing_images, superpixels, bg_masks
...
def make_transforms_asymmetrical(config):
    ...
    """
    transforms = []
    if config["dataset"]["transforms"]["mixed"] is not None:
        for t in config["dataset"]["transforms"]["mixed"]:
            if t.lower() == "resizedcrop":
                transforms.append(
                    ResizedCrop(
                        image_crop_size=config["model"]["image"]["crop_size"],
                        image_crop_ratio=config["model"]["image"]["crop_ratio"],
                    )
                )
...
def make_transforms_asymmetrical_val(config):
    ...
    if config["dataset"]["transforms"]["mixed"] is not None:
        for t in config["dataset"]["transforms"]["mixed"]:
            if t.lower() == "resizedcrop":
                transforms.append(
                    ResizedCrop(image_crop_size=config["model"]["image"]["crop_size"], crop_center=True)
                )
    if not len(transforms):
        return None
    return ComposeAsymmetrical(transforms)
