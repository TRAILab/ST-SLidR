...
class LightningDownstream(pl.LightningModule):
    def __init__(self, model, config):
        ...
        self.train_losses = []
        self.val_losses = []
        self.ignore_index = config["model"]["ignore_index"]
        self.n_classes = config["model"]["n_out"]
        self.epoch = 0
        if config["criteria"]["loss"].lower() == "lovasz":
        ...
        self.log_prefix = log_utils.get_log_prefix(config.get("log_groups", []))

    def configure_optimizers(self):
        if self._config["solver"].get("lr_head", None) is not None:
            ...
            weight_decay_head = self._config["solver"]["weight_decay_head"] if (self._config["solver"]["weight_decay_head"] is not None) else weight_decay
            parameters = [
                {"params": iter(param_group_head), "lr": self._config["solver"]["lr_head"], "weight_decay": weight_decay_head},
                {"params": iter(param_group_trunk)}]
            print(f"==> Head:  #{len(param_group_head)} params with learning rate: {self._config['solver']['lr_head']} and weight_decay: {weight_decay_head}")
            print(f"==> Trunk: #{len(param_group_trunk)} params with learning rate: {self._config['solver']['lr']} and weight_decay: {weight_decay}")
            ...
            if self._config["solver"].get("optimizer") and self._config["solver"]["optimizer"] == 'adam':
        ...
        if self._config["solver"].get("scheduler") and self._config["solver"]["scheduler"] == 'steplr':
            ...
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer, self._config["trainer"]["num_epochs"]
            )
    ...
    def training_step(self, batch, batch_idx):
        if self._config["model"]["freeze_layers"]:
            self.model.eval()
        else:
            self.model.train()
        ...
        self.log(
            f"{self.log_prefix}train_loss", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            f"{self.log_prefix}val_loss", loss, on_epoch=True, prog_bar=True, logger=True, sync_dist=True
        )
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.train_losses = []
        self.val_losses = []
        self.log(f"{self.log_prefix}m_IoU", m_IoU, prog_bar=True, logger=True, sync_dist=False)
        self.log(f"{self.log_prefix}fw_IoU", fw_IoU, prog_bar=True, logger=True, sync_dist=False)
        if self.epoch == self._config["trainer"]["num_epochs"]:
            self.save()
