...
class PretrainDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if self.config["dataset"]["name"].lower() == "nuscenes" and self.config["model"]["pointcloud"]["backbone"] == "minkunet":
            Dataset = NuScenesMatchDataset
        elif self.config["dataset"]["name"].lower() == "nuscenes" and self.config["model"]["pointcloud"]["backbone"] == "voxelnet":
    ...
    def train_dataloader(self):
        ...
        if self.config["trainer"]["num_gpus"]:
            num_workers = self.config["trainer"]["num_threads"] // self.config["trainer"]["num_gpus"]
        else:
            num_workers = self.config["trainer"]["num_threads"]
        if self.config["model"]["pointcloud"]["backbone"] == "minkunet":
