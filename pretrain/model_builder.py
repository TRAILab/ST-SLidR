...
def make_model(config):
    ...
    """
    if config["model"]["pointcloud"]["backbone"] == "voxelnet":
        model_points = VoxelNet(4, config["model"]["n_out"], config)
    else:
        model_points = MinkUNet(1, config["model"]["n_out"], config)
    if config["model"]["image"]["encoder"].find("vit_") != -1:
        model_images = DinoVitFeatureExtractor(config, preprocessing=Preprocessing())
    elif config["model"]["image"]["decoder"] == "dilation":
        model_images = DilationFeatureExtractor(config, preprocessing=Preprocessing())
    elif config["model"]["image"]["decoder"] == "ppkt":
        model_images = PPKTFeatureExtractor(config, preprocessing=Preprocessing())
    else:
        # model with a decoder
        raise Exception(f'Model not found: {config["model"]["image"]["decoder"]}')
